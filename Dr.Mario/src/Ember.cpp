...
namespace Ember {
	...
	void Texture::Draw(const FRect& dest, float angle, const SDL_RendererFlip& flip, const Vec2& rotate_point) {
		if (rotate_point == Vec2({-1, -1})) 
			SDL_RenderCopyExF(renderer->Renderer(), texture, NULL, &dest.rect, angle, NULL, flip);
		else {
			SDL_FPoint rotate = { rotate_point.x, rotate_point.y };
			SDL_RenderCopyExF(renderer->Renderer(), texture, NULL, &dest.rect, angle, &rotate, flip);
		}
	}

	void Texture::Draw(const FRect& dest, const Rect& src, const SDL_RendererFlip& flip, float angle, const Vec2& rotate_point) {
		if (rotate_point == Vec2({ -1, -1 })) 
			SDL_RenderCopyExF(renderer->Renderer(), texture, &src.rect, &dest.rect, angle, NULL, flip);
		else {
			SDL_FPoint rotate = { rotate_point.x, rotate_point.y };
			SDL_RenderCopyExF(renderer->Renderer(), texture, &src.rect, &dest.rect, angle, &rotate, flip);
		}
	}

	void Texture::Draw(const Rect& dest, float angle, const SDL_RendererFlip& flip, const IVec2& rotate_point) {
		if (rotate_point == IVec2({ -1, -1 })) 
			SDL_RenderCopyEx(renderer->Renderer(), texture, NULL, &dest.rect, angle, NULL, flip);
		else {
			SDL_Point rotate = { rotate_point.x, rotate_point.y };
			SDL_RenderCopyEx(renderer->Renderer(), texture, NULL, &dest.rect, angle, &rotate, flip);
		}
	}

	void Texture::Draw(const Rect& dest, const Rect& src, const SDL_RendererFlip& flip, float angle, const IVec2& rotate_point) {
		if (rotate_point == IVec2({ -1, -1 })) 
			SDL_RenderCopyEx(renderer->Renderer(), texture, &src.rect, &dest.rect, angle, NULL, flip);
		else {
			SDL_Point rotate = { rotate_point.x, rotate_point.y };
			SDL_RenderCopyEx(renderer->Renderer(), texture, &src.rect, &dest.rect, angle, &rotate, flip);
		}
	}
